...
$app->match('/students/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "INSERT INTO `students` (`firstname`, `name`, `website`) VALUES (?, ?, ?)";
            $app['db']->executeUpdate($update_query, array($data['firstname'], $data['name'], $data['website']));            
...
$app->match('/students/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "UPDATE `students` SET `firstname` = ?, `name` = ?, `website` = ? WHERE `id` = ?";
            $app['db']->executeUpdate($update_query, array($data['firstname'], $data['name'], $data['website'], $id));            
